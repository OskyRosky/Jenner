...
El objetivo es obtener una ecuación estable, estadísticamente válida y, sobre todo, coherente con la física del comportamiento a fatiga.
...
```{r}
# Modelo completo (full model) — TMN 12.5 mm, 400 με
mod_full_12_400 <- lm(
  log_ciclos ~ 
    pdf_rigidez_inicial_m_pa +
    pdf_rigidez_final_mpa +
...
# --- Data con variable dependiente ---
df <- df_12_400 |> 
  mutate(log_ciclos = log10(pdf_numero_de_ciclos))

# --- Función para ajustar regresión simple ---
ajustar_regresion <- function(var) {
  f <- as.formula(paste0("log_ciclos ~ ", var))
  m <- lm(f, data = df)
...
# --- Ejecutar todas las regresiones ---
resultados_12_400 <- do.call(rbind, lapply(vars, ajustar_regresion))

# --- Ordenar por R² ---
resultados_12_400 <- resultados_12_400 |> arrange(desc(r2))

resultados_12_400

```

Tenemos problemas grandes de Multicolinealidad....
...
**Validación física de coeficientes**
...
E.  Validación del modelo
	...
	•	Diagnóstico de residuos
	•	Q–Q plot
	•	Gráfico valor ajustado vs residual
...
### 4.4 Comparación entre los cuatro modelos

Se analiza:
	•	Diferencias entre TMN 12.5 y TMN 19
	•	Diferencias entre deformación 400 vs 600 μϵ
	•	Efecto mecánico esperado:
	•	más deformación → menos ciclos
	•	TMN mayor → mayor rigidez pero menor sensibilidad
...
Ejemplo:
	•	Qué propiedades resultaron más relevantes
	•	Si el efecto de TMN y deformación es coherente
	•	Si los modelos mostraron estabilidad y baja colinealidad
