...
El Objetivo 1 se orientó a comprender el comportamiento del porcentaje de vacíos de aire medido en las vigas compactadas, tanto desde la perspectiva de su variabilidad dentro del proceso productivo como desde su relación con las propiedades volumétricas de las mezclas. El propósito general fue identificar, por un lado, dónde se origina la variación observada entre mezclas, bloques y vigas, y por otro, qué características del material explican mejor dicha variación.

El análisis se desarrolló mediante dos enfoques complementarios.
En primer lugar, se empleó un modelo mixto jerárquico para respetar la estructura propia del diseño experimental, en el cual las vigas están anidadas dentro de bloques y los bloques dentro de mezclas. Bajo este esquema, la mezcla se consideró como efecto fijo —por representar diseños formulados de manera específica— mientras que los efectos correspondientes al bloque (compactación) y la viga (corte) se trataron como aleatorios. Este modelo permitió descomponer la variabilidad total del porcentaje de vacíos y evaluar la contribución relativa de cada nivel del proceso de fabricación.

En segundo lugar, se ajustó un modelo de regresión lineal múltiple con el fin de estudiar la relación física entre el porcentaje de vacíos y las propiedades volumétricas y de gradación medidas en las propias vigas. A diferencia del análisis jerárquico, este enfoque no evalúa la estructura del proceso, sino la capacidad explicativa que tienen variables como el Gmm, el Gmb, el VMA, el VFA, los vacíos PV y los porcentajes que pasan por diferentes tamices para describir cómo cambian los vacíos cuando varían las características del material. Para ello, se realizó primero una evaluación de colinealidad, seguida del ajuste de un modelo base y un procedimiento de selección apoyado en criterios estadísticos y en la coherencia física de los coeficientes.

En conjunto, ambos análisis ofrecen una visión integral del comportamiento de los vacíos: el modelo mixto permite entender la variación interna del proceso mezcla–bloque–viga, mientras que la regresión aporta una ecuación descriptiva que relaciona el resultado con las propiedades fundamentales del material. Esta sección resume únicamente el enfoque metodológico; las conclusiones de fondo se presentan más adelante en el documento.
