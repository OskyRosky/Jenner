...
El análisis descriptivo del conjunto de datos correspondiente al Objetivo 2 muestra primero que la estructura jerárquica del diseño (mezcla–bloque) se mantiene exactamente igual que en el Objetivo 1: las 24 mezclas tienen diez observaciones cada una, y cada mezcla contiene dos bloques A/B con cinco vigas cada uno. Esto confirma que el diseño experimental está completamente balanceado, lo cual es una condición favorable para los modelos de regresión comparativa que se ajustarán más adelante.

Desde un punto de vista numérico, los parámetros de compactación y propiedades volumétricas mantienen rangos prácticamente idénticos a los observados en el Objetivo 1, tal como se esperaba porque ambos objetivos provienen de los mismos bloques y vigas. La gran diferencia aparece en las variables propias del ensayo de fatiga bajo AASHTO T321: deformación unitaria, rigidez inicial, rigidez final, energía disipada y número de ciclos. Estas variables presentan una distribución mucho más dispersa y, en particular, el número de ciclos exhibe valores muy elevados (hasta niveles de cientos de miles o más), lo cual se refleja en el primer boxplot donde su magnitud domina la escala y oculta el comportamiento del resto de variables. Por esta razón fue necesario generar un segundo boxplot excluyendo esta variable, lo que permite observar con claridad la variabilidad general del resto de parámetros de fatiga.

Los gráficos de correlación ofrecen una primera visión interesante: las variables de fatiga están moderadamente asociadas entre sí, lo que es coherente desde el punto de vista mecánico (por ejemplo, mezclas con mayor rigidez tienden a presentar un comportamiento distinto ante ciclos de carga). Sin embargo, estas correlaciones no alcanzan niveles suficientemente altos como para anticipar problemas inmediatos de multicolinealidad severa. En contraste, las propiedades volumétricas —especialmente VMA, VFA, vacíos y asfalto efectivo— muestran nuevamente la estructura de dependencia interna típica de cualquier mezcla asfáltica, lo que es esperable debido a la relación matemática entre estas propiedades. Esta dependencia será importante al momento de ajustar los modelos de regresión: no impide el ajuste, pero sí requiere una selección cuidadosa de variables para evitar redundancias.

En términos generales, el dataset se comporta de manera coherente y estable, no se detectan valores extremos fuera de rangos físicamente posibles y no existen problemas de balance o estructura que limiten los análisis posteriores. La única variable que requiere tratamiento visual especial es el número de ciclos, debido a su escala muy superior al resto. Por lo demás, la información está en condiciones adecuadas para avanzar hacia la construcción de modelos predictivos diferenciados por TMN y nivel de deformación, tal como requiere el Objetivo 2.
