...
```{r}
# Resumen numérico del % de vacíos
resumen_vacios <- summary(datos_obj1_raw$gbs_percent_vacios_7_1)

data.frame(
  Estadístico = names(resumen_vacios),
  Valor       = as.numeric(resumen_vacios)
) |>
  gt() |>
  tab_header(
    title = "Resumen — gbs_percent_vacios_7_1"
  ) |>
  fmt_number(
    columns = Valor,
    decimals = 4
...
```{r}
# Convertir la tabla ANOVA a data.frame para gt
...
anova_df |>
  ...
  tab_header(
    ...
    subtitle = "Modelo mixto: Mezcla (fijo) + Bloque anidado (aleatorio)"
  ) |>
  fmt_number(
...
vc <- VarCorr(mod_vacios) |> as.data.frame()

# Preparar tabla con varianza, desviación estándar y aporte %
vc_tidy <- vc |>
  transmute(
    Termino = recode(grp,
                     "mezcla_n"             = "Mezcla (intercepto aleatorio)",
                     "mezcla_n:bloque_n"    = "Bloque dentro de mezcla",
                     "Residual"             = "Residuos"),
    Varianza = vcov,
    `Desv. Estándar` = sqrt(vcov),
    `Aporte %` = 100 * vcov / sum(vcov)
  )
...
vc_tidy |>
  ...
  tab_header(
    ...
    subtitle = "Varianza, desviación estándar y porcentaje de contribución"
  ) |>
  fmt_number(
    columns = c(Varianza, `Desv. Estándar`, `Aporte %`),
    decimals = 4
  ) |>
  cols_label(
    Termino = "Término",
  ...
  tab_style(
    ...
    locations = cells_body(columns = Termino)
  )
```
...
coef_step |>
  gt() |>
  tab_header(
    title    = "Modelo de regresión final — Objetivo 1",
    subtitle = "Coeficientes estimados del modelo seleccionado por AIC"
  ) |>
  fmt_number(
...
$$
\widehat{\text{Vacíos}} = 6.732 - 1.245\,\text{Gmb} + 0.310\,\text{VMA} - 0.052\,\text{Polvo/Asfalto}
$$
